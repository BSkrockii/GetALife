...
def register(request):
    ...
    if request.method == 'POST':
        ...
        # add expense
        expense = Budget_expense.objects.create(name = 'expense ' + date.strftime('%B'), description = date.year, month = date.month, account_id = acct.id, expense = 10000)

        # add income
        income = Budget_income.objects.create(name = 'income ' + date.strftime('%B'), description = date.year, month = date.month, account_id = acct.id, income = 10000)
